    ...
    return (
        <div className="professional-admin">
            ...
            <div className="admin-main">
                ...
                {/* --- Section: Customer Orders (Sales) --- */}
                <div className="content-section" style={{ marginTop: '40px' }}>
                    <div className="section-header">
                        <h2>🛍️ Customer Sales</h2>
                    </div>
                    {customerOrders.length === 0 ? (
                        <p className="empty-state" style={{ padding: '20px', color: '#64748b' }}>No product sales yet.</p>
                    ) : (
                        <div className="table-container">
                            <table className="data-table">
                                <thead>
                                ...
                                <tbody>
                                    {customerOrders.map(order => (
                                        <tr key={order._id}>
                                            <td style={{ fontWeight: 'bold' }}>#{order._id.slice(-6).toUpperCase()}</td>
                                            <td>{new Date(order.createdAt).toLocaleDateString()}</td>
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                                                    {order.items.map((item, idx) => (
                                                        <div key={idx} style={{ fontSize: '13px' }}>
                                                            ...
                                                            {item.status === 'cancelled' && <span style={{ color: 'red', fontSize: '11px', marginLeft: '4px' }}>[Cancelled]</span>}
                                                        </div>
                                                    ))}
                                                </div>
                                            </td>
